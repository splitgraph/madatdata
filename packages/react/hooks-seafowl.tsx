...
export { makeAuthHeaders, makeSeafowlHTTPContext };
...
export const makeDefaultAnonymousContext = () => {
  const defaultAnonymousContext = makeSeafowlHTTPContext({
    credential: null,
  });
...
export const DSXContext = createContext<SeafowlDataContext>(
  makeSeafowlHTTPContext()
);
...
export const SqlProvider = ({
  ...
  options: Parameters<typeof makeSeafowlHTTPContext>[0];
}>) => {
  const stableContext = useMemo(() => makeSeafowlHTTPContext(options), []);
